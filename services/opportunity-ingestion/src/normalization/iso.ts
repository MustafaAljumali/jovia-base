...
export function normalizeCurrency(value: string): string | undefined {
  ...
  return ISO_4217.has(normalized) ? normalized : undefined;
}

export function normalizeCountry(value: string): string | undefined {
  const normalized = value.trim().toUpperCase();
  return ISO_3166_ALPHA_2.has(normalized) ? normalized : undefined;
}

export function normalizeLanguage(value: string): string | undefined {
  const normalized = value.trim().toLowerCase();
  return ISO_639_1.has(normalized) ? normalized : undefined;
}
